...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    var css = [
      ...
      /* 备选方案 A：左侧呼吸光带（此前效果，可切回） */
      /*.qjump-scroll::before{content:'';position:absolute;left:0;top:8%;bottom:8%;width:1.5px;border-radius:2px;background:linear-gradient(180deg,transparent,rgba(200,205,212,.55) 18%,rgba(200,205,212,.55) 82%,transparent);opacity:0;transform:scaleY(.6);transition:opacity .3s ease,transform .3s ease}*/
      /*.qjump-scroll:focus-visible::before{opacity:1;transform:scaleY(1);animation:qjump-focus-pulse 2.6s ease-in-out infinite}*/
      /* 当前：底部磨砂光晕 */
      ".qjump-scroll::after{content:'';position:absolute;left:50%;bottom:-10px;transform:translateX(-50%);width:80%;height:70%;border-radius:50%;background:radial-gradient(ellipse at center 60%,rgba(200,205,212,.32),rgba(200,205,212,0) 75%);filter:blur(7px);opacity:0;transition:opacity .35s ease;pointer-events:none}",
      ".qjump-scroll:focus-visible::after{opacity:1;animation:qjump-focus-pulse 2.8s ease-in-out infinite}",
      "@keyframes qjump-focus-pulse{0%,100%{opacity:.35}50%{opacity:.85}}",
