...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    // (v1.3.0 retired the shell.overlay rail; its .qjump-* rail CSS block
    //  was removed here too. Interaction styles live in interactCss below.)
    ...
    var interactCss = [
      ...
      ".qjump-quote{display:flex;align-items:center;gap:8px;width:auto;max-width:100%;margin:0 auto;box-sizing:border-box;padding:8px 10px 8px 18px;border:none;border-radius:8px;background:var(--dsw-alias-interactive-bg-hover-solid);color:var(--dsw-alias-label-primary);font-size:12.5px;line-height:1.5;position:relative}",
      "body[data-ds-dark-theme] .qjump-quote{background:var(--dsw-alias-interactive-bg-hover-solid)}",
      ...
      /* ---- 官方轮次导航兼容修复（v1.3.1） ----
         官方标尺钉在会话滚动区右缘内 12px 处（right:-20px 与容器 padding 恰好抵消），
         会被 dsh-better-sidebar 右面板的左缘拖拽把手（8px，悬出 4px）+ 边框阴影盖掉。
         把整条导航标尺向内挪 12px（right:-8px），预览气泡跟随 frame 定位自动跟着挪。 */
      "nav[class$=\"_frame\"]{right:-8px !important}"
    ].join("");
    ...
    // DSH 0.1.2 adaptation: chat UI moved into @deepseek-ai/dsh-client-ui-chat
    // (all CSS-module hashes rebuilt, e.g. p-xYUq_actions -> hWmORq_actions),
    // and the composer became a Lexical contenteditable (no <textarea>).
    // Therefore: hash class names are matched by [class$="_actions"] /
    // [class$="_bubble"] suffix selectors (hash-agnostic); composer focus and
    // typing detection go through [data-composer-card] / [contenteditable];
    // the draft text is read from the official useInput() contract instead of
    // textarea.value. All legacy selectors remain as fallbacks so one build
    // keeps working on both 0.1.1-rc.x and 0.1.2-rc.x hosts.
    // ======================================================================

    var ACTIONS_SELECTOR = '[class$="_actions"]';       // MessageIconActions row (hash-agnostic)
    var BUBBLE_SELECTOR = '[class$="_bubble"]';         // host user bubble (hash-agnostic)
    var COMPOSER_SELECTOR = "[data-composer-card], [data-input-scroll]";
    var INPUT_TEXTAREA_SELECTOR = '[data-composer-card] [contenteditable="true"], [data-input-scroll] [contenteditable="true"], [data-composer-input], [data-input-scroll] textarea';
    var COPY_BTN_SELECTOR = 'button[aria-label="复制"], button[aria-label="copy"], button[aria-label="Copy"], button[aria-label="Copy message"]';
    ...
    function findUserActionsRow(userEl) {
      ...
      var row = userEl.querySelector(ACTIONS_SELECTOR);
      if (row) return row;
    ...
    function userMessageText(userEl) {
      if (!userEl) return "";
      var bubble = userEl.querySelector(BUBBLE_SELECTOR);
    ...
    function focusComposerTextarea() {
      ...
      if (!ta) {
        ...
        ta = scroll ? scroll.querySelector("textarea, [contenteditable=\"true\"]") : null;
      }
      if (!ta) return;
      ...
      useEffect(function () {
        ...
        var scan = function () {
          ...
          users.forEach(function (userEl) {
            ...
            var anchor = actionsRow.querySelector(COPY_BTN_SELECTOR) || actionsRow.querySelector("button");
            if (!anchor) return;
            ...
            actionsRow.insertBefore(btn, anchor.nextSibling);
          });
        };
      ...
      useEffect(function () {
        ...
        var scheduleShow = function (target) {
          ...
          frame = requestAnimationFrame(function () {
            ...
            if (node.closest("[data-input-scroll], [data-composer-card], [data-composer-input], textarea, [contenteditable=\"true\"], .qjump-sel-pop, .qjump-edit-pop, .qjump-edit-overlay, .qjump-quote")) { setSelection(null); return; }
    ...
    function QJumpQuotePanel(props) {
      ...
      // 0.1.2：composer 是 Lexical contenteditable，草稿文本走官方 useInput 契约；
      // 旧宿主没有该 prop 时退回读 DOM。
      var useInput = props.useInput;
      var draftNow = useInput ? useInput(function (s) { return s && s.draft; }) : undefined;
      var draftRef = useRef("");
      useEffect(function () { if (typeof draftNow === "string") draftRef.current = draftNow; }, [draftNow]);
      ...
      useEffect(function () {
        if (!quote) return;
        var ta = document.querySelector(INPUT_TEXTAREA_SELECTOR) || document.querySelector(COMPOSER_SELECTOR);
        if (!ta) return;
        var sync = function () {
      ...
      //   - Enter（无修饰、非组合输入、焦点在 composer 输入区——含 Lexical contenteditable）
      ...
      useEffect(function () {
        ...
        var attachQuote = function () {
          ...
          if (!draft) {
            var ta = document.querySelector(INPUT_TEXTAREA_SELECTOR);
            if (ta && typeof ta.value === "string") draft = ta.value;
            else if (ta && typeof ta.innerText === "string") draft = ta.innerText.trim();
          }
        ...
        var onKeyDown = function (e) {
          ...
          if (!activeEl) return;
          var inComposer = (activeEl.closest && activeEl.closest(COMPOSER_SELECTOR)) || activeEl.isContentEditable;
        ...
        var onClick = function (e) {
          var t = e.target;
          var btn = t && t.closest
            ? t.closest('button[aria-label="发送消息"], button[aria-label="发送"], button[aria-label="Send message"], button[aria-label="Send"], [data-composer-card] button[type="submit"]')
            : null;
          if (!btn) return;
          attachQuote();
          quoteStore.set(null);
    ...
    function QJumpUserMessage(props) {
      ...
      var handleCopy = function (e) {
        if (copied) return;
        // 只复制手动输入的内容（正文），不包含追问卡片内容
        var copyText = bodyText;
        try {
          var p = navigator.clipboard.writeText(copyText);
      ...
      return React.createElement("div", { className: "qjump-msg-row", "data-time-hover-root": true },
        React.createElement("div", { className: "qjump-msg-stack" },
          images.length > 0 && typeof renderMessageImages === "function" ? renderMessageImages({ images: images, align: "end" }) : null,
    ...
    // ====================================================================
    // Question navigator RAIL RETIRED (v1.3.0): DSH 0.1.2-rc.1 ships an
    // official built-in turn navigator ("轮次导航", TurnNavigatorRail) in
    // the dsh-client-ui-chat package, mounted unconditionally inside
    // ChatView. To avoid two right-edge rails fighting, this plugin no
    // longer registers its own shell.overlay ruler — navigation is left
    // to the official rail (it even handles unloaded-history turns).
    // Everything below remains active: select-to-ask quote panel,
    // edit-and-resend pencil, custom user message renderer.
    // ====================================================================
      ...
      // 注意：注册描述符的 locale 字段是 i18n 命名空间（本渲染器不用宿主词典），
      // 语言码 "zh-CN" 是历史误用，已移除；priority -1 低于内置 0，宿主文档
      // 明确动态/低优先级注册胜出（shadow the shipped frame）。
