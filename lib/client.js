          ...
          function (props) {
            ...
            var collect = useCallback(function () {
              ...
              var list = nodes.map(function (el, index) {
                var bubble = el.querySelector(".qjump-msg-bubble, .gdEzaW_bubble");
                var text = bubble ? bubble.textContent : el.textContent;
                return { index: index, el: el, text: compactText(text) };
              });
              setTurns(list);
              return scrollEl;
            }, []);
